...
async function authenticateDoctor(req, res) {
    try {
        ...
        const token = jwt.sign({_id: doctor._id},'prathamesh',{expiresIn:'1h'})
        res.status(200).send({ message: "Authentication successful", accessedToken:token,doctorID:doctor._id, doctorName: doctor.doctorName });
    } catch (error) {
